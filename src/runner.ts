...
export async function run(env: RunnerEnv): Promise<RunnerResult> {
  ...
  let card: ToolCard;
  try {
    card = JSON.parse(read(cardPath)) as ToolCard;
  } catch (e) {
    write(`::error::could not parse ToolCard at "${cardPath}" — ${(e as Error).message}`);
    return { exitCode: 1, markdown: "", outputWritten: false, commentPosted: false, reason: "malformed ToolCard JSON" };
  }

  const markdown = generateReadme(card, { hideBadges });
  ...
  if (wantsComment) {
    if (!token) {
      ...
      if (!issueNumber) {
        ...
        const body = `### MCP Tool Card README — \`${cardPath}\`\n\n${markdown}\n\n_Generated by [mcp-tool-card-readme-generator-action](https://github.com/mizcausevic-dev/mcp-tool-card-readme-generator-action)._`;
